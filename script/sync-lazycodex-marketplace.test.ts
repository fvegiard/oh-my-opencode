...
async function writePluginFixture(sourceRoot: string, options: WritePluginFixtureOptions = {}): Promise<void> {
  ...
  await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", ".codex"), { recursive: true })
  await writeFile(join(sourceRoot, "packages", "omo-codex", "plugin", ".codex", "config.toml"), 'source = "/local/private/checkout"\n')
  await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", ".opencode"), { recursive: true })
  await writeFile(join(sourceRoot, "packages", "omo-codex", "plugin", ".opencode", "oh-my-openagent.jsonc"), "{}\n")
  await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", ".omo", "evidence"), { recursive: true })
  await writeFile(join(sourceRoot, "packages", "omo-codex", "plugin", ".omo", "evidence", "private.log"), "local private evidence\n")
  await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", ".cursor"), { recursive: true })
  await writeFile(join(sourceRoot, "packages", "omo-codex", "plugin", ".cursor", "environment.json"), "{}\n")
  await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", ".sisyphus", "rules"), { recursive: true })
  await writeFile(join(sourceRoot, "packages", "omo-codex", "plugin", ".sisyphus", "rules", "private.md"), "private\n")
  await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", ".dori-sync"), { recursive: true })
  await writeFile(join(sourceRoot, "packages", "omo-codex", "plugin", ".dori-sync", "state.json"), "{}\n")
  await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", ".playwright-mcp"), { recursive: true })
  await writeFile(join(sourceRoot, "packages", "omo-codex", "plugin", ".playwright-mcp", "session.json"), "{}\n")
  await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", "test-injection"), { recursive: true })
  await writeFile(join(sourceRoot, "packages", "omo-codex", "plugin", "test-injection", "payload.json"), "{}\n")
  await writeFile(join(sourceRoot, "packages", "omo-codex", "plugin", ".env"), "LOCAL_ONLY=value\n")
  await writeFile(join(sourceRoot, "packages", "omo-codex", "plugin", ".env.local"), "LOCAL_ONLY=value\n")
}
...
describe("sync-lazycodex-marketplace", () => {
  test("#given marketplace sync #when copying plugin bundle #then emits self-contained mcp paths", async () => {
    ...
    await expectPathMissing(join(lazycodexRoot, "plugins", "omo", ".codex"))
    await expectPathMissing(join(lazycodexRoot, "plugins", "omo", ".opencode"))
    await expectPathMissing(join(lazycodexRoot, "plugins", "omo", ".omo"))
    await expectPathMissing(join(lazycodexRoot, "plugins", "omo", ".cursor"))
    await expectPathMissing(join(lazycodexRoot, "plugins", "omo", ".sisyphus"))
    await expectPathMissing(join(lazycodexRoot, "plugins", "omo", ".dori-sync"))
    await expectPathMissing(join(lazycodexRoot, "plugins", "omo", ".playwright-mcp"))
    await expectPathMissing(join(lazycodexRoot, "plugins", "omo", "test-injection"))
    await expectPathMissing(join(lazycodexRoot, "plugins", "omo", ".env"))
    await expectPathMissing(join(lazycodexRoot, "plugins", "omo", ".env.local"))
  })
